...
(function (global, $) {
    ...
    function tlpShowHideScMeta() {
        ...
        if (layout) {
            ...
            isWc = layout.match(/^wc/i);
            isWcIsotope = layout.match(/^wc-isotope/i);
            isWcCarousel = layout.match(/^wc-carousel/i);
    ...
    function detailLinkEffect() {
        ...
        if (detailPageLink == "yes") {
            ...
            $(".field-holder.detail-page-link-type,.field-holder.popup-type").hide();
        }
        linkTypeEffect();
    }
