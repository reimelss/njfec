(function ($) {
    ...
    function ajaxPagination(container, isotopeM) {
        $(".rt-pagination.rt-ajax ul li").on('click', 'a', function (e) {
            ...
            data = data + "&paged=" + paged + "&l4toggle=" + l4toggle;
            data = data + "&action=tpgLoadMorePreview&" + rttpg.nonceID + "=" + rttpg.nonce;
                ...
                success: function (data) {
                    ...
                    if (!data.error) {
                        if (typeof isotopeM === "undefined") {
                            ...
                            container.children(".rt-row").promise().done(function () {
                                container.children(".rt-row").html(data.data).show().animate({opacity: 1});
                            });
                        } else {
    ...
    function loadMoreButton($this, $isotope, container, layout, IsoButton) {
        ...
        data = data + "&paged=" + paged + "&action=tpgLoadMorePreview" + "&l4toggle=" + l4toggle;
        $.ajax({
            type: "post",
            url: rttpg.ajaxurl,
            data: data,
