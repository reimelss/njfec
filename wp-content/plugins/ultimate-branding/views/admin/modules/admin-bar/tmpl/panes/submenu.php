		...
		<div class="sui-row">
			<div class="sui-col">
				<div class="sui-form-field branda-submenu-title branda-admin-bar-submenu-title">
					...
					<span class="hidden">This field can not be empty!</span>
				</div>
			</div>
			<div class="sui-col">
				<div class="sui-form-field branda-submenu-target">
					<label for="branda-submenu-target-{{{data.id}}}" class="sui-label">Open link in</label>
					<div class="sui-side-tabs sui-tabs">
						<div class="sui-tabs-menu">
							<label class="sui-tab-item <# if ( 'new' === data.target ) { #>active<# } #>">
<input type="radio" name="branda[submenu][{{{data.id}}}][target]" value="new" data-name="" data-tab-menu="" <# if ( 'new' === data.target ) { #>checked="checked"<# } #> />New Tab</label>
							<label class="sui-tab-item <# if ( 'current' === data.target ) { #>active<# } #>">
<input type="radio" name="branda[submenu][{{{data.id}}}][target]" value="current" data-name="" data-default="" data-tab-menu="" <# if ( 'current' === data.target ) { #>checked="checked"<# } #> />Same Tab</label>
						</div>
					</div>
				</div>
			</div>
...
<input type="radio" name="branda[submenu][{{{data.id}}}][url]" value="admin" data-name="submenu-url-{{{data.id}}}" data-tab-menu="branda-admin-bar-submenu-url-{{{data.id}}}-admin" <# if ( 'admin' === data.url ) { #>checked="checked"<# } #>  />Admin Page</label>
					<label class="sui-tab-item <# if ( 'site' === data.url ) { #>active<# } #>">
<input type="radio" name="branda[submenu][{{{data.id}}}][url]" value="site" data-name="submenu-url-{{{data.id}}}" data-tab-menu="branda-admin-bar-submenu-url-{{{data.id}}}-site" <# if ( 'site' === data.url ) { #>checked="checked"<# } #> />Site Page</label>
					<label class="sui-tab-item <# if ( 'custom' === data.url ) { #>active<# } #>">
<input type="radio" name="branda[submenu][{{{data.id}}}][url]" value="custom" data-name="submenu-url-{{{data.id}}}" data-tab-menu="branda-admin-bar-submenu-url-{{{data.id}}}-custom" <# if ( 'custom' === data.url ) { #>checked="checked"<# } #> />External</label>
				</div>
				<div class="sui-tabs-content">
					<div class="sui-tab-boxed <# if ( 'admin' === data.url ) { #>active<# } #>" data-tab-content="branda-admin-bar-submenu-url-{{{data.id}}}-admin">
						...
						<p class="sui-tab-boxed">URL is relative to <b>http://wordpress/wp-admin/</b>
</p>
					</div>
