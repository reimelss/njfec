...
function ub_url( $extended ) {
	global $branda_url;
	return ub_get_url_valid_shema( $branda_url ) . $extended;
}
...
function ub_files_url( $extended ) {
	return ub_url( 'inc/' . $extended );
}

function ub_files_dir( $extended ) {
	return ub_dir( 'inc/' . $extended );
}
 ...
 */
function ub_get_option( $option, $default = false, $mode = 'normal' ) {
...
function get_ub_activated_modules( $mode = 'normal' ) {
	$modules = ub_get_option( 'ultimatebranding_activated_modules', array(), $mode );
	/**
	 * For mode 'raw' do nor use filters.
	 *
	 * @since 3.0.0
...
function ub_register_uninstall_hook() {
	ub_load_single_module( 'utilities/data.php' );
