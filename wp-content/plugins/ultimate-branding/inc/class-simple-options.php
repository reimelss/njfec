...
if ( ! class_exists( 'Simple_Options' ) ) {
	...
	class Simple_Options {
						...
						case 'checkboxes':
							...
							if ( isset( $data['options'] ) && is_array( $data['options'] ) ) {
								...
								foreach ( $data['options'] as $checkbox_value => $checkbox_label ) {
									...
									$content .= sprintf(
										'<input type="checkbox" id="%s_%s" name="simple_options[%s][%s][%s]" value="1" class="%s" %s />',
										esc_attr( $html_id ),
						...
						case 'sui-tab-icon':
							...
							if (
								...
								foreach ( $content_tabs[ $section_key ][ $data['slave-class'] ] as $slave_id => $slave ) {
									...
									$content .= sprintf(
										'<div class="sui-tab-boxed%s branda-tab-%s-%s" data-tab-content="%s">',
		...
		public function ajax_reset_section() {
			...
			$option_name = ub_get_option_name_by_module( $module );
			if (
				empty( $section )
				|| empty( $module )
				|| empty( $nonce )
