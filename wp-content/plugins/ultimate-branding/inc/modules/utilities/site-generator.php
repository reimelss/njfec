...
if ( ! class_exists( 'Branda_Site_Generator' ) ) {
	...
	class Branda_Site_Generator extends Branda_Helper {
		...
		public function upgrade_options() {
			$value = ub_get_option( 'site_generator_replacement' );
			if ( ! empty( $value ) ) {
				$this->set_value( 'generator', 'text', $value );
				ub_delete_option( 'site_generator_replacement' );
			}
			$value = ub_get_option( 'site_generator_replacement_link' );
			if ( ! empty( $value ) ) {
				$this->set_value( 'generator', 'link', $value );
				ub_delete_option( 'site_generator_replacement_link' );
			}
		}
		...
		public function replace( $gen, $type ) {
			 ...
			 * mask
			 */
			$mask = '<meta name="generator" content="%1$s" />';
			$masks = array(
