...
if ( ! class_exists( 'Branda_Comments_Control' ) ) {
	...
	class Branda_Comments_Control extends Branda_Helper {
		...
		public function comments_open_check( $status, $post_id ) {
			...
			if ( $this->disallow_comments ) {
				...
				if ( array_key_exists( $post_type, $disabled ) ) {
					$value = $this->get_value( 'settings', 'by_blacklist', 'off' );
					if ( 'on' === $value ) {
						return false;
					}
		...
		private function is_blacklisted_ip() {
			$blacklist = $this->get_value( 'settings', 'blacklist', array() );
		...
		public function upgrade_options() {
			 ...
			 */
				...
				$value['settings']['blacklist'] = ub_get_option( 'limit_comments_denied_ips' );
			...
			$value = ub_get_option( $option_name_cpt );
			if ( ! empty( $value ) ) {
				$this->set_value( 'settings', 'available_post_types', $value );
				ub_delete_option( $option_name_cpt );
			}
		}
		...
		public function limit_comments_flood_filter( $flood_die, $time_lastcomment, $time_newcomment ) {
			 ...
			 * get settings
			 */
			$whitelist = $this->get_option( 'rules', 'whitelist', array() );
			$blacklist = $this->get_option( 'settings', 'blacklist', array() );
		...
		protected function set_options() {
				...
				'settings' => array(
					...
					'fields' => array(
						...
						'blacklist' => array(
							...
							'placeholder' => __( 'Enter your IP blacklist here...', 'ub' ),
							'display' => 'sui-tab-content',
							'master-value' => 'off',
						),
						'by_blacklist' => array(
				...
				'rules' => array(
					...
					'fields' => array(
						'whitelist' => array(
							...
							'classes' => array( 'large-text' ),
							'placeholder' => __( 'Enter your IP whitelist here...', 'ub' ),
						),
					),
					'master' => array(
						'section' => 'settings',
			...
			/**
			 * Remove options from single site installation
			 */
			if ( ! $this->is_network ) {
				unset( $options['settings']['fields']['blacklist'] );
				unset( $options['settings']['fields']['by_blacklist'] );
				unset( $options['rules'] );
			}
