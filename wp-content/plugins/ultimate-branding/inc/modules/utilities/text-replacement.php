...
if ( ! class_exists( 'Branda_Text_Replacement' ) ) {
	class Branda_Text_Replacement extends Branda_Helper {
		...
		public function upgrade_options() {
			...
			$value = ub_get_option( 'translation_table' );
			if ( empty( $value ) ) {
				return;
			}
			$list = array();
			...
			ub_delete_option( 'translation_table' );
		}
				...
				array(
					...
					'fields' => array(
						'find' => array(
							...
							'placeholder' => __( 'E.g. Posts', 'ub' ),
							'value' => isset( $item['find'] )? $item['find']:'',
							'required' => true,
						),
						'replace' => array(
							'label' => __( 'Replace with', 'ub' ),
							'placeholder' => __( 'E.g. Blog Articles', 'ub' ),
		...
		private function calculate_regexp( $data ) {
			...
			return '/' . str_replace( '/','\/', stripslashes( $data['find'] ) ) . '/';
		}
		...
		public function ajax_delete() {
			...
			$this->set_value( 'list', $_POST['id'], null );
			$message = array(
				'class' => 'success',
