...
if ( ! class_exists( 'Branda_Images' ) ) {
	...
	class Branda_Images extends Branda_Helper {
		...
		public function upgrade_options() {
			...
			$data = ub_get_option( 'ub_favicons' );
			if ( ! empty( $data ) ) {
				...
				if ( isset( $data['global'] ) ) {
					...
					if ( isset( $data['global']['favicon_meta'] ) ) {
						$value['favicon']['favicon_meta'] = $data['global']['favicon_meta'];
					}
		...
		protected function set_options() {
			...
			if ( ! $this->is_network ) {
				unset( $options['favicons']['fields']['subsites'] );
			}
		 ...
		 * @since 3.0.0
		 */
		private function calculate_favicon( $url, $size, $favicon, $meta ) {
			if (
				is_array( $meta )
				&& isset( $meta[0] )
				&& ! empty( $meta[0] )
			...
			if ( ! empty( $favicon ) ) {
				$meta = wp_get_attachment_image_src( $favicon );
				if (
					is_array( $meta )
					&& isset( $meta[0] )
					&& ! empty( $meta[0] )
		...
		public function get_site_icon_url( $url, $size, $blog_id ) {
			...
			if ( $this->is_network ) {
				...
				if ( is_main_site( $blog_id ) ) {
					$favicon = $this->get_value( 'favicon', 'favicon', false );
					$meta  = $this->get_value( 'favicon', 'favicon_meta', false );
					return $this->calculate_favicon( $url, $size, $favicon, $meta );
				}
				/**
				 * subsites
				 */
				$value = $this->get_value( 'favicon', 'subsites', 'off' );
				if ( 'force' === $value ) {
					$meta  = $this->get_value( 'favicon', 'favicon_meta', false );
					return $this->calculate_favicon( $url, $size, $favicon, $meta );
				}
				...
				if (
					...
					if ( isset( $value[ $blog_id ]['image'] ) ) {
						$favicon = $value[ $blog_id ]['image'];
					}
					if ( isset( $value[ $blog_id ]['meta'] ) ) {
						$meta = $value[ $blog_id ]['meta'];
					}
				}
				return $this->calculate_favicon( $url, $size, $favicon, $meta );
			}
		...
		public function change_blavatar_icon() {
			...
			foreach ( (array) $wp_admin_bar->user->blogs as $blog ) {
				$icon = $this->get_site_icon_url( false, false, $blog->userblog_id );
				if ( empty( $icon ) ) {
					continue;
				}
				$css .= sprintf(
		...
		public function get_favicon( $blog_id = null ) {
			return $this->get_site_icon_url( false, false, $blog_id );
		}
		...
		public function get_fs_limit() {
			...
			foreach ( $this->roles as $role => $role_name ) {
				if ( in_array( $role, $current_user->roles ) ) {
					if ( ! isset( $limits[ $role ] ) || empty( $limits[ $role ] ) ) {
						continue;
					}
		...
		public function limits() {
			...
			$max_current_quant = floor( $max / $quant );
		...
		private function get_select_sizes( $slug, $value, $max, $select_unit, $wp_max, $raw ) {
			...
			foreach ( $this->quant as $unit => $size ) {
				...
				$content .= sprintf(
					'<option value="%s" data-max="%d" data-value="%d" data-size="%d" %s>%s</option>',
					esc_attr( $unit ),
					esc_attr( $max / $size ),
					esc_attr( $value / $size ),
					esc_attr( $size ),
