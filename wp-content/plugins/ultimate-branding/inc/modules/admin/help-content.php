...
if ( ! class_exists( 'Branda_Admin_Help_Content' ) ) {
	...
	class Branda_Admin_Help_Content extends Branda_Helper {
		...
		public function upgrade_options() {
			...
			$old = ub_get_option( 'admin_help_content' );
			if ( ! empty( $old ) ) {
				$data = array(
					'settings' => array(
						'merge_panels' => 'off',
				...
				if ( isset( $old['tabs'] ) ) {
					ub_update_option( 'ub_admin_help_items', $old['tabs'] );
				}
				ub_delete_option( 'admin_help_content' );
			}
			/**
			 * Migrate items
			 */
			$data = ub_get_option( 'ub_admin_help_items' );
			if ( ! empty( $data ) && is_array( $data ) ) {
				...
				ub_delete_option( 'ub_admin_help_items' );
			}
		}
