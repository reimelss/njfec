...
if ( ! class_exists( 'Branda_Color_Schemes' ) ) {
	...
	class Branda_Color_Schemes extends Branda_Helper {
		...
		public function upgrade_options() {
			...
			$ucs_visible_color_schemes = ub_get_option( 'ucs_visible_color_schemes' );
			...
			$data = array(
				...
				'defaul_color_scheme' => array(
					'scheme' => ub_get_option( 'ucs_default_color_scheme', 'do-not-change' ),
				),
				'force_color_scheme' => array(
					'scheme' => ub_get_option( 'ucs_force_color_scheme', 'do-not-force' ),
				),
				'settings' => array(
			 ...
			 * Delete old values
			 */
			ub_delete_option( 'ucs_default_color_scheme' );
			ub_delete_option( 'ucs_force_color_scheme' );
			ub_delete_option( 'ucs_visible_color_schemes' );
			/**
		...
		private function convert_ultimate_scheme() {
			...
			foreach ( $map as $old_name => $name ) {
				...
				$value = ub_get_option( $old_name, $default );
				/**
				 * delete old value
				 */
		...
		public function ajax_save() {
			...
			$data = array();
			foreach ( $this->defaults as $key => $default ) {
				if ( ! isset( $_POST['branda'][ $key ] ) ) {
					continue;
				}
				$value = $_POST['branda'] [ $key ];
				if ( empty( $value ) ) {
					continue;
				}
				/**
