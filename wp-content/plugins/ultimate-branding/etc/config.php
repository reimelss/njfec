...
function ub_get_modules_list( $mode = 'full' ) {
	global $wp_version, $branda_network;
	$modules = array(
		...
		'admin/menu.php' => array(
			...
			'description' => __( 'This module allows your to customize the admin menu. Enable the Link Manager, remove the permalinks configuration option or remove "Dashboard" link from admin panel for users without site (in WP Multisite).', 'ub' ),
			'group' => 'admin',
			'public' => true,
			'wp' => '3.5',
			'options' => array( 'ub_admin_menu' ),
		),
		/**
		 * data
		 *
